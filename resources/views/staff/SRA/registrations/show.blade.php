...
<div class="mb-4">
    <a href="{{ route('staff.registrations') }}" style="font-size:13px;color:var(--g-600);text-decoration:none;">
        <i class="ph ph-arrow-left me-1"></i> Back to Overseas Jobseekers
    </a>
    ...
    <div class="card-body p-3">
        ...
        @else
            <form action="{{ route('staff.registrations.apply', $registration->jobseeker_registrations_id) }}" method="POST" class="d-flex gap-2 flex-wrap">
                @csrf
                <select name="job_id" class="form-select form-select-sm" style="max-width:400px;border-color:var(--n-200);font-size:13px;" required>
                    <option value="">— Select a job posting —</option>
                    @foreach($openJobs as $job)
                    <option value="{{ $job->job_qualifications_id }}">{{ $job->title }} — {{ $job->company->company_name ?? 'None' }}</option>
                    @endforeach
                </select>
                <button type="submit" class="btn btn-sm fw-semibold"
                    style="background:var(--g-600);color:#fff;border:none;border-radius:8px;font-size:12px;padding:8px 18px;">
                    <i class="ph-fill ph-paper-plane-tilt me-1"></i> Apply
                </button>
            </form>
        @endif
    </div>
